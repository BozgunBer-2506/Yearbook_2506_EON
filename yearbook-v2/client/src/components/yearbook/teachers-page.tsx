...
export function TeachersPage({ teachers }: TeachersPageProps) {
  ...
  return (
    <div className="relative z-10 p-6 lg:p-8 h-full flex flex-col">
        ...
        {teachers.map((teacher) => (
          <div key={teacher.id} className="flex flex-col items-center group">
            ...
            <div className="relative mb-3">
              {/* Animated glow ring */}
              <div className="absolute -inset-1 bg-gradient-to-r from-cyan-400 via-purple-400 to-cyan-400 rounded-full opacity-75 blur-sm group-hover:opacity-100 transition-opacity" />
              ...
              <div className="relative w-16 h-16 lg:w-20 lg:h-20 rounded-full overflow-hidden border-2 border-cyan-400/50">
                {teacher.profile_picture_url ? (
                  <img
                    src={`/images/${teacher.profile_picture_url}`}
        ...
        <div className="relative">
          ...
          <div className="relative bg-slate-800/50 backdrop-blur-sm rounded-lg p-4 lg:p-6 border border-cyan-500/20">
            ...
            <p className="text-slate-300/90 text-sm lg:text-base leading-relaxed italic font-light">
              Sevgili mezunlarımız, bu yıl boyunca gösterdiğiniz azim ve başarıyı 
              gururla takip ettik. Sizlerle çalışmak bizim için büyük bir zevkti.
              Gelecekteki yolculuğunuzda başarılar dileriz.
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Decorative elements */}
      <div className="absolute bottom-20 left-8 w-32 h-32 border border-cyan-500/10 rounded-full" />
      <div className="absolute top-40 right-8 w-24 h-24 border border-purple-500/10 rounded-full" />
    </div>
  );
}
