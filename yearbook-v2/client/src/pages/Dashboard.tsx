    ...
    return (
      <div className="app">
        ...
        <footer style={{ textAlign: 'center', padding: '12px 16px', fontSize: '0.72rem', color: 'rgba(160,200,232,0.5)', borderTop: '1px solid rgba(0,229,204,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '16px', flexWrap: 'wrap' }}>
          <a href="https://thebozgun.com" target="_blank" rel="noopener noreferrer" style={{ color: 'rgba(0,229,204,0.6)', textDecoration: 'none' }}>© 2026 The_Bozgun</a>
          ...
          {page === 'course' && !selectedTeacher && !selectedStudent && (
            <div className="content-page">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                {teachers.map((t, idx) => (
                  <div key={t.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', padding: '12px', background: 'rgba(0,229,204,0.04)', border: '1px solid rgba(0,229,204,0.15)', borderRadius: '8px', textAlign: 'center', gridColumn: teachers.length % 2 !== 0 && idx === teachers.length - 1 ? '1 / -1' : undefined,
width: teachers.length % 2 !== 0 && idx === teachers.length - 1 ? '50%' : undefined,
margin: teachers.length % 2 !== 0 && idx === teachers.length - 1 ? '0 auto' : undefined }}>
        ...
        {page !== 'students' && !selectedTeacher && !selectedStudent && (
          <div className="nav-mobile">
            ...
            <button className="nav-btn" onClick={nextPage}
              disabled={currentIndex === pages.length - 1}>▶</button>
          </div>
        )}
      </main>
    </div>
